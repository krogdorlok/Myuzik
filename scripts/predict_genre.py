...
def predict_genre(audio_filepath: str, classifier_path: str = None):
    ...
    try:
        audio_time_series = load_and_preprocess_audio(
            audio_filepath, settings.VGGISH_SAMPLE_RATE, settings.AUDIO_DURATION
        )
    ...
    try:
        ...
        logging.info(
            f"Prediction complete. Genre: {predicted_genre}, Confidence: {confidence:.4f}"
        )
...
def main():
    ...
    parser.add_argument(
        "audio_file", type=str, help="Path to the audio file to classify."
    )
    parser.add_argument(
        "--model",
        type=str,
