...
def create_classifier(classifier_type: str = settings.CLASSIFIER_TYPE, **kwargs):
    ...
    if classifier_type == "logistic_regression":
        ...
        # Note: multi_class param removed in sklearn 1.9+, auto-detected now
        classifier = LogisticRegression(solver="lbfgs", max_iter=1000, **kwargs)
        return classifier
...
def train_classifier(classifier, X_train, y_train):
    ...
    """
    logging.info(
        f"Training {classifier.__class__.__name__} on {X_train.shape[0]} samples..."
    )
...
def save_classifier(
    classifier, label_mapping: dict, model_path: str = settings.CLASSIFIER_MODEL_PATH
):
    """
    ...
    try:
        ...
        model_data = {"classifier": classifier, "label_mapping": label_mapping}
