...
def load_and_preprocess_audio(
    filepath: str, target_sr: int, duration: int
) -> np.ndarray:
    """
    ...
    try:
        ...
        )  # Load with original SR, keep stereo for now
        ...
        if sr != target_sr:
            audio = librosa.resample(
                y=audio, orig_sr=sr, target_sr=target_sr, res_type="kaiser_fast"
            )
...
if __name__ == "__main__":
    ...
    logging.info(
        "Audio processor module ready. Run scripts/extract_features.py for full pipeline test."
    )
