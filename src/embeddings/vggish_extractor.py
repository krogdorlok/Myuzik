...
def load_vggish_model() -> tf.keras.Model:
    ...
    if _vggish_model is None:
        logging.info(
            f"Loading VGGish model from TensorFlow Hub: {settings.VGGISH_MODEL_URL}"
        )
        try:
...
def extract_embedding(audio_time_series: np.ndarray, sr: int) -> np.ndarray:
    ...
    try:
        ...
        logging.debug(
            f"Input waveform shape: {audio_tensor.shape}, dtype: {audio_tensor.dtype}"
        )
        ...
        logging.debug(
            f"Output embeddings shape: {embeddings.shape}, dtype: {embeddings.dtype}"
        )
...
if __name__ == "__main__":
    ...
    logging.info(
        f"Testing VGGish extractor with dummy audio (shape: {dummy_audio.shape}, sr: {test_sr})"
    )
    ...
    try:
        ...
        if extracted_embedding.shape == (128,):
            logging.info(
                f"Successfully extracted VGGish embedding. Shape: {extracted_embedding.shape}"
            )
            logging.info("VGGish extractor module ready.")
        else:
            logging.error(
                f"Unexpected embedding shape: {extracted_embedding.shape}. Expected (128,)."
            )
